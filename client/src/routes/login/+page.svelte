<script lang="ts">
  ...
  // Two steps in one route: enter a mobile, then enter the code texted to
  // it. A separate /auth/action route (what email-link sign-in needed) has
  // no equivalent here — there's no link to land on, so the whole flow
  // stays on this page and in this component's state.
  let mobile = $state('');
  let code = $state('');
